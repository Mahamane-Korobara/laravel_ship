        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                    <x-ui.button type="button" wire:click="importRepo('{{ $repo['full_name'] }}')" variant="secondary" size="sm" wire:loading.attr="disabled" wire:target="importRepo('{{ $repo['full_name'] }}')" class="border border-slate-700/60 bg-slate-900/60 text-slate-200 hover:bg-slate-800">
                        <span wire:loading.remove wire:target="importRepo('{{ $repo['full_name'] }}')" class="inline-flex items-center gap-2">
                            <x-icon name="lucide-arrow-right" class="h-4 w-4" />
                            Importer
                        </span>
                        <span wire:loading wire:target="importRepo('{{ $repo['full_name'] }}')" class="inline-flex items-center gap-2">
                            <x-ui.spinner size="sm" />
                            Import...
                        </span>
                    </x-ui.button>
